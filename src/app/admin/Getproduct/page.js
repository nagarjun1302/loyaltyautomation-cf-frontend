...
export default function ProductListPage() {
  ...
  return (
    <AdminShell
      ...
      actions={
        <a href="/admin/Addproduct" className="rounded-md bg-teal-700 px-4 py-2.5 text-sm font-black text-white hover:bg-teal-800">
          Add Product
        </a>
      }
    >
      <div className="mb-5 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        <label className="relative block">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search product, category, brand or model..." className="focus-ring h-12 w-full rounded-md border border-slate-200 bg-slate-50 pl-12 pr-4 text-sm" />
        </label>
      </div>

      {loading && <div className="rounded-md border border-slate-200 bg-white p-8 text-center font-bold text-slate-500">Loading products...</div>}
      ...
      <div className="grid gap-4">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => (
            <article key={product._id} className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
              <div className="grid gap-0 lg:grid-cols-[220px_1fr]">
                <div className="border-b border-slate-200 bg-slate-50 p-4 lg:border-b-0 lg:border-r">
                  <div className="aspect-square overflow-hidden rounded-md border border-slate-200 bg-white">
                    {product.productimage ? <img src={uploadUrl(product.productimage)} alt={product.title} className="h-full w-full object-contain" /> : null}
                  </div>
                </div>
                ...
                <div className="p-4">
                  {editProductId === product._id ? (
                    <div className="grid gap-4 md:grid-cols-2">
                      <SearchableSelect
                        ...
                        onChange={(value) => setEditableProductData({ ...editableProductData, category: value })}
                      />
                      <SearchableSelect
                        label="Brand"
                        ...
                        onChange={(value) => setEditableProductData({ ...editableProductData, Brand: value })}
                      />
                      {editableFields.map(([name, label, type]) => (
                        <div key={name} className={type === "textarea" ? "md:col-span-2" : ""}>
                          <label className="mb-2 block text-sm font-bold text-slate-700">{label}</label>
                          {type === "textarea" ? (
                            <textarea value={editableProductData[name] || ""} onChange={(event) => setEditableProductData({ ...editableProductData, [name]: event.target.value })} rows={4} className="focus-ring w-full resize-none rounded-md border border-slate-200 p-3" />
                          ) : (
                            <input type={type || "text"} value={editableProductData[name] || ""} onChange={(event) => setEditableProductData({ ...editableProductData, [name]: event.target.value })} className="focus-ring h-11 w-full rounded-md border border-slate-200 px-3" />
                          )}
                        </div>
                      ))}
                      <div className="md:col-span-2">
                        <label className="mb-2 block text-sm font-bold text-slate-700">Replace Product Images</label>
                        <label className="block rounded-md border border-dashed border-slate-300 bg-slate-50 p-4 transition hover:border-teal-300">
                          ...
                          <input type="file" accept="image/*" multiple onChange={handleEditImages} className="w-full text-sm text-slate-600 file:mr-3 file:rounded-md file:border-0 file:bg-slate-950 file:px-3 file:py-2 file:text-sm file:font-bold file:text-white" />
                          <span className="mt-2 block text-xs font-bold text-slate-500">Selecting images replaces the current product gallery. Leave empty to keep existing images.</span>
                        </label>
                        {editableImagePreviews.length > 0 && (
                          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                            {editableImagePreviews.map((preview, index) => (
                              <div key={preview.url} className="rounded-md border border-slate-200 bg-white p-2">
                    ...
                    <>
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          <p className="text-xs font-black uppercase tracking-wide text-teal-700">{product.category || "Industrial"}</p>
                          <h2 className="mt-1 text-xl font-black text-slate-950">{product.title || "Untitled product"}</h2>
                          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{product.description || "No description available."}</p>
                        </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {editProductId === product._id ? (
                      <>
                        <button onClick={handleSave} className="rounded-md bg-green-600 px-4 py-2 text-sm font-black text-white hover:bg-green-700">Save</button>
                        <button onClick={cancelEdit} className="inline-flex items-center gap-2 rounded-md border border-slate-200 px-4 py-2 text-sm font-black text-slate-700 hover:bg-slate-50"><X className="h-4 w-4" /> Cancel</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => handleEdit(product._id)} className="inline-flex items-center gap-2 rounded-md bg-slate-950 px-4 py-2 text-sm font-black text-white hover:bg-slate-800"><Edit3 className="h-4 w-4" /> Edit</button>
                        <button onClick={() => handleDelete(product._id)} className="inline-flex items-center gap-2 rounded-md border border-red-200 px-4 py-2 text-sm font-black text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /> Delete</button>
                      </>
                    )}
                  </div>
                </div>
          ...
          !loading && <div className="rounded-md border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">No products found.</div>
        )}
      </div>
    </AdminShell>
  );
...
function SearchableSelect({ label, value, onChange, options, placeholder }) {
  ...
  return (
    <div className="relative">
      <label className="mb-2 block text-sm font-bold text-slate-700">{label}</label>
      <button type="button" onClick={() => setOpen((current) => !current)} className="focus-ring flex h-11 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 text-left">
      ...
      {open && (
        <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl">
          <label className="relative block border-b border-slate-100">
            ...
            <input value={searchValue} onChange={(event) => setSearchValue(event.target.value)} placeholder="Search..." className="h-11 w-full px-9 text-sm outline-none" />
          </label>
